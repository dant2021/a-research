...
class KModel_modified(torch.nn.Module):
    '''
    ...
    class Output:
        ...
        pred_dur_raw: Optional[torch.FloatTensor] = None
    ...
    def forward(
        ...
        ref_dur: Optional[torch.LongTensor] = None
        ...
        if ref_dur is not None:
            indices = torch.repeat_interleave(torch.arange(input_ids.shape[1], device=self.device), ref_dur)
        else:
        ...
        return self.Output(audio=audio, pred_dur=pred_dur, pred_dur_raw=duration) if return_output else audio
